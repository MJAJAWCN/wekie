"""
读取后处理后的 infer_fixed_no_data_answer_rule.txt
-> 按 item id 合并 window SER
-> 如果所有窗口预测一致，直接采用一致结果
-> 只有发生预测冲突时，才启用“上下左右文完整的非 O 预测优先”
-> 否则回退 core 优先 + 多数投票
"""
...
def vote_final(votes):
    ...
    if len(raw_preds) == 1:
        final_pred = next(iter(raw_preds))
        return final_pred, PRED_ID.get(final_pred, 0), votes, Counter([final_pred]), "all_votes_same"
    ...
    if context_non_o_votes:
        ...
        return final_pred, final_pred_id, context_non_o_votes, counts, "conflict_context_non_o"
    ...
    return final_pred, final_pred_id, usable_votes, counts, "conflict_core_or_all"
...
def merge_records(records):
    ...
    for item_id, items in grouped_items.items():
        ...
        if merge_reason == "conflict_context_non_o":
...
def write_outputs(merged_by_group, stats, output_root, infer_path):
    ...
    manifest = {
        ...
        "merge_strategy": "all_same_direct_else_context_non_o_then_core",
        "groups": [],
        "global_stats": stats,
    }
