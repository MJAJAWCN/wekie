"""
读取后处理(把无资料改为answer)后的 window SER infer 文件
-> 按原始 item id 合并重复窗口预测
-> core item 优先
-> 多数投票
-> 平票 QUESTION > ANSWER > HEADER > O
-> 输出每张逻辑大表一份 merged SER JSON
"""
import argparse
import json
...
def vote_final(votes):
    ...
    counts = Counter(normalize_pred(v.get("pred")) for v in usable_votes)
    ...
    return final_pred, PRED_ID.get(final_pred, 0), usable_votes, counts
...
def merge_records(records):
    ...
    for item_id, items in grouped_items.items():
        votes = [make_vote(item, item["_image_path"]) for item in items]
        final_pred, final_pred_id, usable_votes, counts = vote_final(votes)
    ...
    stats = {
        ...
        "conflict_items_sample": conflict_items[:50],
    }
...
def main():
    ...
    for group_id in sorted(merged_by_group):
        ...
        print(f"group={group_id:03d} items={len(merged_by_group[group_id])} counts={dict(counts)}")
