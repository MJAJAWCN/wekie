...
if __name__ == '__main__':
    ...
    with open(
        ...
        for idx, info in enumerate(infer_imgs):
            ...
            # 输入是 23.jpg 时，自动给 KIE 传入：E:\wepipelinestage3\wepipelineStage2\wepipeline\workspace\mineru_outputs_for_kie\23_ocr_info.json"""
            else:
                img_path = info
                data = {'img_path': img_path}

                mineru_ocr_dir = config["Global"].get("mineru_ocr_dir", None)
                if mineru_ocr_dir:
                    image_stem = os.path.splitext(os.path.basename(img_path))[0]
                    data["ocr_info_path"] = os.path.join(
                        mineru_ocr_dir,
