...
class VQATokenLabelEncode(object):
    ...
    # 如果传进来了 ocr_info_path，就读 MinerU 的 JSON；如果没传，才走 PaddleOCR 自己的 OCR
    def _load_ocr_info(self, data):
        if self.infer_mode:
            if "ocr_info" in data:
                return data["ocr_info"]

            if "ocr_info_path" in data and data["ocr_info_path"]:
                with open(data["ocr_info_path"], "r", encoding="utf-8") as f:
                    return json.load(f)
